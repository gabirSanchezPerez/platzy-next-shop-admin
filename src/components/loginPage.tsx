...
export default function LoginPage() {
  ...
  const submitHanlder = (event: React.FormEvent) => {
    ...
    auth
      ...
      .catch((err: any) => auth.setFail(err.message));
  };
  return (
    <>
      <div className="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8">
          <div>
            <Image className="mx-auto h-12 w-auto" src="https://tailwindui.com/img/logos/workflow-mark-indigo-600.svg" alt="Workflow" width="300" height="200" />
            <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Sign in to your account</h2>
          </div>
          {auth?.fail !== "" && (
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
              <strong className="font-bold">Upss! </strong>
              <span className="block sm:inline">{auth?.fail}</span>
            </div>
          )}
